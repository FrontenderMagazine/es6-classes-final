...
является объект. Значение свойства _prototype_ оператор _new_ использует 
как прототип для создаваемых объектов.

![][3]

**Для начала рассмотрим псевдо-метод _constructor_.** Этот метод является 
...
*   Статические методы `Foo.*` доступны для записи и настройки, но
    не для перечисления. Доступность для запси позволяет динамически вносить
    изменения в них.
...
#### 4.3 Почему мы не можем наследовать встроенные конструкторы в ЕS5? {#why_can’t_you_subclass_built-in_constructors_in_ES5}
...
не можете использовать _Array_ чтобы настроить экземпляр который создал _MyArray_.
...
*   Инициализация также происходит в базовом конструкторе, конструктор 
    ...
    вместо того, чтобы создавать собственный объект и отдавать его конструктору 
    базового класса, чтобы тот его создавал.


#### 4.4 Ссылка на базовые свойства в методах {#referring_to_super_properties_in_methods}
...
хранится этот метод, _домашний объект_. Например, 
_ColorPoint.prototype_ - это _домашний объект_ для _ColorPoint.prototype.toString()_.
...
Способ 3 показывает как в ECMAScript 6 обрабатываются базовые вызовы. Этот 
подход поддерживается [двумя внутренними *привязками*][10], которые имеют 
_окружения_ функций (_окружение_ обеспечивает хранилище, так называемые 
_привязки_, для переменных контекста):

*   _[[thisValue]]_: Эта внутрення привязка также есть и в ECMAScript 5 и
    хранит значение переменной _this_.
*   `[[HomeObject]]`: Относится к домашнему объекту окружения функции. Заполняется
...
помощью так называемого _шаблона разновидности_:
...
Следующие get'еры возвращают _this_, который означает, что такие методы 
...
*   Arrow functions are specialized for non-method callbacks, where them
    picking up the _this_ of their surrounding method or constructor is an 
    advantage. Without _this_, they don’t make much sense as methods and they 
    throw an exception when invoked via _new_.

*   Определения метода позволяют использовать _super_, создав свойство
...
классы обеспечивают основу для такой функциональности как миксины, значения
объектов (где различные объекты равны, если они имеют одинаковое содержание) 
и константные классы (которые создают неизменяемые экземпляры).

#### 7.3 Нужны ли классы JavaScript'у? {#does_javascript_need_classes%3F}
...
придется иметь дело с неортодоксальные механизмами наследования в JavaScript.
