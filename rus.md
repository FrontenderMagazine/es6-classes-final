[Недавно][1], TC39 определили финальную семантику классов в ECMAScript 6 [[2]](#anchor-2).
...
конструтора вызывать конструктор базового класса через `super()`, о котором 
...
_итератор_ [[4]](#anchor-4). Это означает, что его содержимое может быть итерировано 
...
Если вы определите метод с «*» в начале, то получите _метод генератор_ [[4]](#anchor-4).
...
*   _Определения методов_ (в объектах, заданых через литерал, или классах,
...
Вы также можете наследоваться от `Array`, экзмепляры которого правильно 
работают с `length`:
...
*   `this` инициализируется в производных конструкторах и значит, что если
...
**Первое припятствие: инициализация.** `this`,который вы передаете в 
...
*   `[[thisValue]]`: Эта внутрення привязка также есть и в ECMAScript 5 и
...
*   `[[HomeObject]]`: Относится к домашнему объекту состояния функции. 
    ...
    функции, использововашие `super`.  И привязка и свойство являются новыми 
    в ECMAScript 6.
...
*   `[[Construct]]`: Все функции конструткора (включая также созданные 
...
*   Если конструктор наследника `Array` ничего не делает, он наследует
    `Array[Symbol.species]`. Это свойство является get'ером, который 
    возвращает `this`.
...
Следующие get'еры возвращают `this`, это означает, что такие методы 
