...
свойствами `Foo.prototype`. Эти, как правило, методы, и наследуются 
экземплярами `Foo`.
...
<span id="note-1" class="note">1.</span> [Реформа создания экземпляров: в последний раз][13], слайды Аллена Вирфс-Брока (Allen Wirfs-Brock)
